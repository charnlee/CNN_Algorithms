...
def train_model_process(model,train_dataloader,val_dataloader,num_epochs):
    ...
    for epoch in range(num_epochs):
        ...
        for step,(b_x,b_y) in enumerate(train_dataloader):
            ...
            batch_time_use = time.time() - s
            print(f"------已完成第{step}批次训练------用时:{batch_time_use//60:.0f}m{batch_time_use%60:.0f}s")
        # 取数据加载器里面的数据
        for step, (b_x, b_y) in enumerate(val_dataloader):
            ...
            val_num += b_x.size(0)
            batch_time_use = time.time() - s
            print(f"------已完成第{step}批次验证------用时:{batch_time_use//60:.0f}m{batch_time_use%60:.0f}s")
        # 把每一轮的损失加到损失列表里
    ...
    torch.save(best_model_wts,'../VGGNet/best_model.pth')
...
if __name__ == "__main__":
    ...
    train_process = train_model_process(vgg,train_dataloader,val_dataloader,5)
